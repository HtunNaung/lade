...
// const zerothrust_auth = process.env.ZERO_AUTH || 'eyJhIjoiNmIwYzRiZDczMjQ4Y2IxNTYyMTdmN2QyNzZlOWE5ZjAiLCJ0IjoiMDgxZjI0MTQtNDAxNi00ZGQ3LWE5NmYtOTJlYTNlMjJjOGU5IiwicyI6Ik1ERXdZVEZrTVRrdFl6UmxNeTAwTW1GakxUazVaREV0Tm1ZNU5UUXhZelkyTXpCayJ9'; // Removed as it's only used by the external 'server' binary

// --- Removed direct 'exec' calls for 'server' binary ---
// These lines are typically handled by a separate startup script or process manager
// exec('chmod +x server');
...
server.listen(port, () => { //
    console.log('Server listening on port:', port); // Simplified log call
    console.log('VLESS Proxy UUID:', uuid); // Simplified log call
    console.log('Access home page at: http://localhost:' + port); // Simplified log call
});

server.on('error', err => { //
    console.error('Server Error:', err); // Simplified error log call
});
